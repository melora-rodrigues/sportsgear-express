...
if (isset($_GET['cr_id'])) {
    ...
    $crid = $_GET['cr_id'];
    $action = $_GET['action'];
    $st = $admin->ret("SELECT * FROM `cart` WHERE `cr_id`='$crid'");
    $userCart = $st->fetch(PDO::FETCH_ASSOC);
    $cartQuantity = $userCart['cr_quantity'];
    $cartDays = $userCart['cr_days'];
    if ($action == "increment" || $action == 'decrement') {
        ...
        $stmt = $admin->cud("UPDATE `cart` SET `cr_quantity`='$updatedCartQuantity' WHERE `cr_id`= '$crid'", "saved");
    } else {
        if ($action == "incrementDays") {
            $updatedCartDays = $cartDays + 1;
        } else if ($action == "decrementDays") {
            $updatedCartDays = $cartDays - 1;
        }
        $stmt = $admin->cud("UPDATE `cart` SET `cr_days`='$updatedCartDays' WHERE `cr_id`= '$crid'", "saved");
    }
